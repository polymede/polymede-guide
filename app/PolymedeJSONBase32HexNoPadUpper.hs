...
module PolymedeJSONBase32HexNoPadUpper(polymedeJSON) where
...
-- we wrap the ByteString contents of the file in a newtype wrapper to make it easy to specify the
-- the JSON instances, down to the exact multibase codec we want to favour for encoding
newtype FileContents =
  ...
  deriving (FromJSON,ToJSON) 
    via UsingMultibaseCodec Base32HexNoPadUpper FileContents
